...
type SendContactReceiptEmailParams = {
  toEmail: string;
  toName: string;
  subject: string;
};
...
type SendContactInternalNotificationParams = {
...
export async function sendContactReceiptEmail({ toEmail, toName, subject }: SendContactReceiptEmailParams) {
  const fromEmail = process.env.AWS_SES_FROM_EMAIL;
  const fromName = process.env.AWS_SES_FROM_NAME;
  const configurationSet = process.env.AWS_SES_CONFIGURATION_SET;
  ...
  const sourceAddress = `${fromName} <${fromEmail}>`;
  const client = getSesClient();
  const subjectLine = subject?.trim() ? `We received your message: ${subject}` : "We received your message";

  const htmlBody = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #0f172a;">
      <h2 style="margin-bottom: 12px;">We received your message</h2>
      <p>Hi ${toName || "there"},</p>
      <p>Thanks for reaching out to ListHit. Our team will review your message and respond as soon as possible.</p>
      <p style="margin: 18px 0; padding: 12px 16px; background-color: #f8fafc; border-radius: 8px;">
...
Thanks for reaching out to ListHit. Our team will review your message and respond as soon as possible.
...
export async function sendContactInternalNotification({ payload }: SendContactInternalNotificationParams) {
  const fromEmail = process.env.AWS_SES_FROM_EMAIL;
  const fromName = process.env.AWS_SES_FROM_NAME;
  const configurationSet = process.env.AWS_SES_CONFIGURATION_SET;
  const supportEmail = "support@listhit.io";
  ...
  const sourceAddress = `${fromName} <${fromEmail}>`;
  const client = getSesClient();
  const phoneLine = payload.phone ? payload.phone : "—";
  ...
  const htmlBody = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #0f172a;">
      ...
      <ul style="padding-left: 18px;">
        ...
        <li><strong>Submitted at:</strong> ${payload.receivedAt}</li>
      </ul>
      <p style="margin-top: 12px;"><strong>Message:</strong></p>
...
Submitted at: ${payload.receivedAt}
...
${payload.message}`;
  ...
  const command = new SendEmailCommand({
    ...
    Destination: { ToAddresses: [supportEmail] },
    Message: {
      Subject: { Data: `New contact form submission: ${payload.subject}`, Charset: "UTF-8" },
      Body: {
        Html: { Data: htmlBody, Charset: "UTF-8" },
        Text: { Data: textBody, Charset: "UTF-8" },
      },
