...
export async function POST(req: NextRequest) {
  ...
  try {
    const hasUser = await hasUserWithEmail(supabase, email);
...
async function hasUserWithEmail(supabase: ReturnType<typeof createClient>, email: string) {
  let page = 1;
  let lastPage = 1;

  while (page <= lastPage) {
    const { data, error } = await supabase.auth.admin.listUsers({ page, perPage: listUsersPerPage });
